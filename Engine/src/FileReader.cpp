...
bool REC::FileReader::LoadFromFile(const std::string& filePath, REC::FileResourceCreateInfo::FileType type, char separator)
{
	m_Header.clear();
	m_Rows.clear();
	m_ColumnIndex.clear();
	...
	switch (type)
	{
	case REC::FileResourceCreateInfo::FileType::NONE:
		return false;

	case REC::FileResourceCreateInfo::FileType::CSV:
	{
	...
	case REC::FileResourceCreateInfo::FileType::JSON:
		//...
		return true;
	}
	return false;
