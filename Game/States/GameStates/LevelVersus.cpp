...
Game::LevelVersus::LevelVersus(const REC::EngineContext& context)
	: LevelState(context)
{}

void Game::LevelVersus::Enter()
{
	LevelState::Enter();
	SetPlayerMode(PlayerMode::Versus);
	...
	player2GO->AddComponent<REC::AnimationStateComponent>(std::make_unique<BalloomFloatingState>(player2GO));
